// @ts-ignore - Export conflicts// @ts-ignore
/**
 * T035: Public profiles logging in packages/public-profiles/src/logging/ProfilesLogger.ts
 *
 * Specialized logger for public profile, portfolio, and social sharing events
  */

import { ProfilesLogger as BaseProfilesLogger, profilesLogger } from '@cvplus/core';

// Re-export the profiles logger
export { profilesLogger };
export default profilesLogger;